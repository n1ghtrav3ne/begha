...
import { PersianDate, Core } from "@/composables/date-picker/core-date-picker";
// ************************ Types ************************
import type {
  Obj,
  Attrs,
  ...
  Shortcuts,
  DefaultDate,
} from "@/composables/date-picker/types";
// ************************ Components ************************
import PDPArrow from "./PDPArrow.vue";
import PDPIcon from "./PDPIcon.vue";
import PDPAlt from "./PDPAlt.vue";
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      interval: 0 as any,
      showDays: true as boolean,
    };
  },
  computed: {
    getTodayDate(): string {
      const dayOfWeek = moment.utc().format("ddd");
      const mounthNumber = moment.utc().format("jMM");

      return (
        "امروز " +
        moment().format("jD") +
        " " +
        this.getMounthName(mounthNumber) +
        " " +
        moment().format("jYYYY")
      );
    },
    ...
    attrs(): Attrs {
      ...
      for (const key in this.$attrs) {
        try {
          // const [group, attr] = key.match(/(div|label|alt|picker|firstInput|secondInput)-(.*)/) as unknown as [void, keyof Attrs, string];
          ...
          for (let day = 0; day < 7; day++) {
            if (emptyCells) {
              ...
              month[week][day] = {
                ...
                date: moment(
                  this.onDisplay!.clone().addMonth(i).date(showDay).toDate()
                ).format("YYYY-MM-DD"),
                startRange:
                ...
                inRange:
                  ...
                  this.core
                    ...
                    .isBetween(
                      ...(this.selectedDates.map((date) => date.toString()) as [
                        string,
                        string
                      ])
                    ),
    ...
    formats(): Formats {
      ...
      return {
        ...
        alt:
          (this.attrs.alt.format as string) || this.format || format[this.type],
      };
    },
  ...
  methods: {
    ...
    checkDate(date: unknown, part: CalendarPart | TypePart) {
      let from, to;
      ...
      if (from && to) return (date as PersianDate).isBetween(from, to, "[]");
    },
    isInDisable(date: PersianDate, disable?: Disable): boolean {
      ...
      if (Core.isString(disable)) {
        ...
        return (disable as unknown as (date: PersianDate) => boolean)(date);
    ...
    checkDatesEntered() {
      let firstDate = this.datesEntered[0];

      if (firstDate) {
        this.selectedDates[0] = this.core
          .clone()
          .parse(moment(firstDate as MomentInput).format("jYYYY/jM/jD"))
          .calendar("jalali");

        this.addDisplay(firstDate);
      }
      if (this.datesEntered.length > 1) {
        let secondDate = this.datesEntered[1];
        if (secondDate) {
          this.selectedDates[1] = this.core
            .clone()
            .parse(moment(secondDate as MomentInput).format("jYYYY/jM/jD"))
            .calendar("jalali");
          this.addDisplay(secondDate);
        }
      }
    },
    addDisplay(date: String) {
      this.onDisplay = this.core.fromJalali({
        year: moment(date as MomentInput).format("jYYYY"),
        month: moment(date as MomentInput).format("jM"),
      });
    },
    ...
    changeLocale(locale: string | null = null): void {
      if (locale) {
    ...
    onClickOutside() {
      if (!this.show) this.showDatePicker = false;
    },
    stopChangeTime() {
    ...
    showSelectedDates() {
      let startDate = this.datesEntered[0] as MomentInput;
      let endDate = this.datesEntered[1] as MomentInput;
      if (startDate)
        this.displayValue[0] = moment(startDate)
          .format
          // this.$i18n.locale == "fa" ? "jYYYY/jM/jD" : "YYYY/M/D"
          ();
      if (endDate) this.displayValue[1] = moment(endDate).format("jYYYY/jM/jD");
    },

    getMounthName(value: string): string {
      var mounth = "";
